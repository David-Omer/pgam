...
def postprocess_results(
    ...
    if not isinstance(info_save, Mapping):
        info_save = {}

    def _normalize_kernel_x(x):
        arr = np.asarray(x)
    ...
    for cc in range(len(full_fit.var_list)):
        ...
        results["x_kernel"][cc] = _normalize_kernel_x(results["x_kernel"][cc])
        ...
        results["reduced_x_kernel"][cc] = _normalize_kernel_x(
            results["reduced_x_kernel"][cc]
        )
